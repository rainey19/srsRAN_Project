...
bool radio_session_xtrx_impl::set_tx_gain_unprotected(unsigned int port_idx, double gain_dB)
{
...
bool radio_session_xtrx_impl::set_rx_gain_unprotected(unsigned int port_idx, double gain_dB)
{
...
bool radio_session_xtrx_impl::set_tx_freq(unsigned int port_idx, radio_configuration::lo_frequency frequency)
{
  if (port_idx >= tx_port_map.size()) {
    fmt::print(
        "Error: transmit port index ({}) exceeds the number of ports ({}).\n", port_idx, (int)tx_port_map.size());
...
bool radio_session_xtrx_impl::set_rx_freq(unsigned int port_idx, radio_configuration::lo_frequency frequency)
{
  if (port_idx >= rx_port_map.size()) {
    fmt::print("Error: receive port index ({}) exceeds the number of ports ({}).\n", port_idx, (int)tx_port_map.size());
    return false;
  ...
  sampling_rate_hz(radio_config.sampling_rate_hz), async_executor(async_executor_), notifier(notifier_)
{
  ...
  std::vector<radio_xtrx_rx_stream::stream_description> rx_stream_description_list;
  ...
  for (unsigned stream_idx = 0; stream_idx != radio_config.tx_streams.size(); ++stream_idx) {
    ...
    radio_xtrx_tx_stream::stream_description stream_description = {};
  ...
  for (unsigned stream_idx = 0; stream_idx != radio_config.rx_streams.size(); ++stream_idx) {
    ...
    // Prepare stream description.
    radio_xtrx_rx_stream::stream_description stream_description = {};
    stream_description.id                                      = stream_idx;
    stream_description.otw_format                              = radio_config.otw_format;
    stream_description.args                                    = stream.args;
...
std::unique_ptr<radio_session> radio_factory_xtrx_impl::create(const radio_configuration::radio& config,
                                                              task_executor&                    async_task_executor,
                                                              radio_notification_handler&       notifier)
{
  std::unique_ptr<radio_session_xtrx_impl> session =
      std::make_unique<radio_session_xtrx_impl>(config, async_task_executor, notifier);
...
radio_config_xtrx_config_validator radio_factory_xtrx_impl::config_validator;

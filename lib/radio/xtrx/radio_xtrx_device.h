...
static bool radio_xtrx_device_validate_freq_range(const uhd::freq_range_t& range, double freq)
{
...
static bool radio_xtrx_device_validate_gain_range(const uhd::gain_range_t& range, double gain)
{
...
// TODO
  bool set_rx_rate(double rate)
  {
    ...
    return safe_execution([this, rate]() {
      ...
      if (!radio_xtrx_device_validate_freq_range(range, rate)) {
...
// TODO
                                                        ...
                                                        const radio_xtrx_tx_stream::stream_description& description)
  {
    std::unique_ptr<radio_xtrx_tx_stream> stream =
        std::make_unique<radio_xtrx_tx_stream>(usrp, description, async_executor, notifier);
...
// TODO
  ...
  {
    std::unique_ptr<radio_xtrx_rx_stream> stream = std::make_unique<radio_xtrx_rx_stream>(usrp, description, notifier);
...
// TODO
  bool set_tx_gain(unsigned ch, double gain)
  {
    ...
    return safe_execution([this, ch, gain]() {
      ...
      if (!radio_xtrx_device_validate_gain_range(range, gain)) {
...
// TODO
  bool set_tx_freq(uint32_t ch, const radio_configuration::lo_frequency& config)
  {
    ...
    return safe_execution([this, ch, &config]() {
      ...
      if (!radio_xtrx_device_validate_freq_range(range, config.center_frequency_hz)) {

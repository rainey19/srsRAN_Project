...
#include "radio_config_xtrx_validator.h"
...
/// Describes a radio session based on XTRX that also implements the management and data plane functions.
class radio_session_xtrx_impl : public radio_session,
...
private:
  ...
  /// Enumerates possible XTRX session states.
  ...
  /// Wraps the XTRX device functions.
  radio_xtrx_device device;
  ...
  static_vector<std::unique_ptr<radio_xtrx_tx_stream>, RADIO_MAX_NOF_PORTS> tx_streams;
  /// Vector or receive streams.
  static_vector<std::unique_ptr<radio_xtrx_rx_stream>, RADIO_MAX_NOF_PORTS> rx_streams;
...
public:
  /// Constructs a radio session based on XTRX.
  radio_session_xtrx_impl(const radio_configuration::radio& radio_config,
                         task_executor&                    async_executor,
...
class radio_factory_xtrx_impl : public radio_factory
{
public:
  // See interface for documentation
...
private:
  static radio_config_xtrx_config_validator config_validator;
};

...
public:
  ...
  bool on_transmit(lime::StreamMeta& metadata, uint64_t time_spec, bool is_empty, bool tail_padding)
  {
    std::unique_lock<std::mutex> lock(mutex);
    switch (state) {
      ...
      case states::START_BURST:
        ...
        if (!is_empty) {
          ...
          metadata.flushPartialPacket = tail_padding;
  ...
  void stop(lime::StreamMeta& metadata)
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state == states::IN_BURST) {
      metadata.flushPartialPacket = true;

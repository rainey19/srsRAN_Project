...
/// Validates the given F1AP configuration. Returns true on success, otherwise false.
static bool validate_f1ap_appconfig(const f1ap_appconfig& config)
{
  // only check for non-empty F1AP address and default port
  if (config.cu_addr.empty() or config.port != 38472) {
    return false;
  }
  return true;
}
...
bool srsran::validate_appconfig(const du_appconfig& config)
{
  ...
  if (!validate_f1ap_appconfig(config.f1ap_cfg)) {
    return false;
  }

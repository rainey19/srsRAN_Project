...
du_console_helper::du_console_helper(io_broker& io_broker_) :
...
du_console_helper::~du_console_helper()
{
  io_broker_handle.unregister_fd(STDIN_FILENO);
}

void du_console_helper::stdin_handler(int fd)
{
...
void du_console_helper::handle_command(const std::string& command)
{
  if (command == "q") {
    raise(SIGTERM);
  } else if (command == "t") {
...
void du_console_helper::print_help()
{
...
void du_console_helper::set_cells(const span<du_cell_config>& cells_)
{
  cells = cells_;
}

void du_console_helper::on_app_starting()
{
  print_available_radio_factories();
  fmt::print("\n--== srsRAN gNB (commit {}) ==--\n\n", get_build_hash());
}

void du_console_helper::on_app_running()
{
...
void du_console_helper::on_app_stopping()
{
  fmt::print("Stopping ..\n");
}

unsigned du_console_helper::derive_ssb_arfcn(const du_cell_config& cell)
{

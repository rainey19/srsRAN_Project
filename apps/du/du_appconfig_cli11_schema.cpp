...
static void configure_cli11_log_args(CLI::App& app, log_appconfig& log_params)
{
  ...
  app.add_option(
         "--all_level", log_params.all_level, "Default log level for PHY, MAC, RLC, SDAP, and GTPU")
      ->capture_default_str()
      ->check(level_check);
...
static void configure_cli11_pcap_args(CLI::App& app, pcap_appconfig& pcap_params)
{
  ...
  app.add_option("--f1ap_enable", pcap_params.mac.enabled, "Enable F1AP packet capture")->always_capture_default();
}

static void configure_cli11_f1ap_args(CLI::App& app, f1ap_appconfig& f1ap_params)
{
  app.add_option("--addr", f1ap_params.cu_addr, "F1AP IP address")->check(CLI::ValidIPV4)->required();
  app.add_option("--port", f1ap_params.port, "F1AP port")->capture_default_str()->check(CLI::Range(20000, 40000));
  app.add_option("--bind_addr", f1ap_params.bind_addr, "Local IP address to bind for F1AP connection")
      ->check(CLI::ValidIPV4);
  app.add_option("--sctp_rto_initial", f1ap_params.sctp_rto_initial, "SCTP initial RTO value");
  app.add_option("--sctp_rto_min", f1ap_params.sctp_rto_min, "SCTP RTO min");
  app.add_option("--sctp_rto_max", f1ap_params.sctp_rto_max, "SCTP RTO max");
  app.add_option("--sctp_init_max_attempts", f1ap_params.sctp_init_max_attempts, "SCTP init max attempts");
  app.add_option("--sctp_max_init_timeo", f1ap_params.sctp_max_init_timeo, "SCTP max init timeout");
}
...
void srsran::configure_cli11_with_du_appconfig_schema(CLI::App& app, du_appconfig& gnb_cfg)
{
  ...
  // F1AP section.
  CLI::App* f1ap_subcmd = app.add_subcommand("f1ap", "F1AP parameters")->configurable();
  configure_cli11_f1ap_args(*f1ap_subcmd, gnb_cfg.f1ap_cfg);
